...
from clients.clientmix import ClientMix
...
class ServerMix():
    def __init__(self, train_ids, validation_ids, test_ids, dataset_name, note):
        ...
        self.logger = Logger(f'{dataset_name}_mix_{test_ids}_{note}')
        self.train_clients = [ClientMix(self.dataset_name, i, self.logger) for i in self.train_ids]
        self.validation_clients = [ClientMix(self.dataset_name, i, self.logger) for i in self.validation_ids]
        self.test_clients = [ClientMix(self.dataset_name, i, self.logger) for i in self.test_ids]
