...
class ClientMulti():
    def __init__(self, dataset_name, cid, logger):
        ...
        self.cid = cid
        self.logger = logger
        file_path = f'./data_{dataset_name}/id/{int(cid)}.json'
    ...
    def per_train(self, rnd, weights):
    ...
    def multi_train(self, rnd, weights, classes):
        tf.keras.backend.clear_session()
        model = generate_model(self.dataset_name)
        model.set_weights(weights)
        data_x, data_y = self.filter_dataset_by_class_list(self.data_x, self.data_y, classes)
        support_x, support_y, query_x, query_y = self._sample_two_chunks_k1_k2_shots(data_x, data_y, self.num_shots, self.num_shots)
        # save query samples for future meta_update
    ...
    def multi_update(self, rnd, weights, classes):
        tf.keras.backend.clear_session()
        model = generate_model(self.dataset_name)
        model.set_weights(weights)
        self.outer_optimizer = tf.keras.optimizers.Adam(learning_rate=self.outer_lr)
        for _ in range(self.outer_repetition):
            with tf.GradientTape() as t_outer:
                logits = model(self.query_x, training=True)
                loss = self.loss_fn(self.query_y, logits)
            grad = t_outer.gradient(loss, model.trainable_variables)
            self.outer_optimizer.apply_gradients(zip(grad, model.trainable_variables))
        return model.get_weights()
    ...
    def _split_into_k_shots_and_remainder(self, data_x, data_y, k):
        ...
        for i in available_classes:
            indices_in_class = np.nonzero(data_y == i)[0].tolist()
    ...
    def filter_dataset_by_class_list(self, data_x, data_y, class_list):
        indices = np.argwhere(np.isin(data_y, class_list)).flatten()
